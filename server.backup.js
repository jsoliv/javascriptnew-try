...
// let connectionString = 'mongodb+srv://todoAppUser:tarsila@cluster0-bcdnw.mongodb.net/PhrasalVerbs?retryWrites=true' // this line could be inside mongodeb.connect, but we decided to keep it separete so our code gets easier to read
...
mongodb.connect(connectionString, {useNewUrlParser: true, useUnifiedTopology: true}, function(err, client) {
    db = client.db()
    app.listen(80) // step 4 telling what port the localhost, or the host, must be listening to, that means http://localhost:3000/em
  ...
  <div class="container">
    ...
    <ul class="list-group pb-5">
      ${items.map(function(item){ // calling each item in the array
          ...
          <div>
            <button data-id=${item._id} class="edit-me btn btn-secondary btn-sm mr-1">Edit</button> <! linking mongodb item ID to to the Edit bottons !>
...
app.post('/update-item', function (req, res) { //define portion to be received from the site, accoring to the code in brower.js
  db.collection('items').findOneAndUpdate({_id: new mongodb.ObjectID(req.body.id)}, {$set: {text: req.body.text}}, function() { // sending data to mongodb
    res.send("Success")
  })
})
